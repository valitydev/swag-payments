...
%% basic handler
-module(swag_server_parties_handler).
...
is_authorized(
    Req0,
    State = #state{
        operation_id  = 'ActivatePartyByID' = OperationID,
        logic_handler = LogicHandler,
        context       = Context
    }
) ->
...
is_authorized(
    Req0,
    State = #state{
        operation_id  = 'SuspendPartyByID' = OperationID,
        logic_handler = LogicHandler,
        context       = Context
    }
) ->
...
get_request_spec('SuspendPartyByID') ->
    [
        {'X-Request-ID', #{
            source => header,
            rules  => [{type, 'binary'}, {max_length, 32}, {min_length, 1}, true
...
get_response_spec('ActivatePartyByID', 400) ->
    {'DefaultLogicError', 'DefaultLogicError'};

get_response_spec('ActivatePartyByID', 401) ->
    undefined;
...
get_response_spec('GetPartyByID', 400) ->
    {'DefaultLogicError', 'DefaultLogicError'};

get_response_spec('GetPartyByID', 401) ->
    undefined;
...
get_response_spec('SuspendPartyByID', 400) ->
    {'DefaultLogicError', 'DefaultLogicError'};

get_response_spec('SuspendPartyByID', 401) ->
    undefined;

get_response_spec('SuspendPartyByID', 404) ->
    {'GeneralError', 'GeneralError'};
