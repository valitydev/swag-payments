...
%% basic handler
-module(swagger_customers_handler).
...
allowed_methods(
    Req,
    State = #state{
        operation_id = 'GetCustomerEvents'
    }
) ->
    {[<<"GET">>], Req, State};
...
is_authorized(
    Req0,
    State = #state{
        operation_id  = 'CreateBinding' = OperationID,
        logic_handler = LogicHandler,
        context       = Context
    }
) ->
...
is_authorized(
    Req0,
    State = #state{
        operation_id  = 'GetBinding' = OperationID,
        logic_handler = LogicHandler,
        context       = Context
    }
) ->
...
is_authorized(
    Req0,
    State = #state{
        operation_id  = 'GetBindings' = OperationID,
        logic_handler = LogicHandler,
        context       = Context
    }
) ->
...
is_authorized(
    Req0,
    State = #state{
        operation_id  = 'GetCustomerEvents' = OperationID,
        logic_handler = LogicHandler,
        context       = Context
    }
) ->
...
get_request_spec('CreateBinding') ->
    [
        {'X-Request-ID', #{
            source => header,
...
get_request_spec('GetCustomerEvents') ->
    [
        {'X-Request-ID', #{
            source => header,
            rules  => [{type, 'binary'}, {max_length, 32}, {min_length, 1}, true
...
get_response_spec('CreateBinding', 201) ->
...
get_response_spec('CreateBinding', 404) ->
    {'GeneralError', 'GeneralError'};

get_response_spec('CreateBinding', 409) ->
...
get_response_spec('GetBinding', 200) ->
    {'CustomerBinding', 'CustomerBinding'};

get_response_spec('GetBinding', 400) ->
    {'DefaultLogicError', 'DefaultLogicError'};

get_response_spec('GetBinding', 401) ->
    undefined;

get_response_spec('GetBinding', 404) ->
    {'GeneralError', 'GeneralError'};

get_response_spec('GetBindings', 200) ->
    {'list', 'CustomerBinding'};

get_response_spec('GetBindings', 400) ->
    {'DefaultLogicError', 'DefaultLogicError'};

get_response_spec('GetBindings', 401) ->
    undefined;

get_response_spec('GetBindings', 404) ->
    {'GeneralError', 'GeneralError'};
...
get_response_spec('GetCustomerEvents', 200) ->
    {'list', 'CustomerEvent'};

get_response_spec('GetCustomerEvents', 400) ->
    {'DefaultLogicError', 'DefaultLogicError'};

get_response_spec('GetCustomerEvents', 401) ->
    undefined;
