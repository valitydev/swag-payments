...
%% basic handler
-module(swag_server_shops_handler).
...
allowed_methods(
    Req,
    State = #state{
        operation_id = 'SuspendShopForParty'
    }
) ->
    {[<<"PUT">>], Req, State};
...
is_authorized(
    Req0,
    State = #state{
        operation_id  = 'SuspendShopForParty' = OperationID,
        logic_handler = LogicHandler,
        context       = Context
    }
) ->
...
get_request_spec('GetShopByIDForParty') ->
    [
        {'X-Request-ID', #{
            source => header,
            rules  => [{type, 'binary'}, {max_length, 32}, {min_length, 1}, true
...
get_request_spec('SuspendShopForParty') ->
    [
        {'X-Request-ID', #{
            source => header,
            rules  => [{type, 'binary'}, {max_length, 32}, {min_length, 1}, true
...
get_response_spec('ActivateShopForParty', 400) ->
    {'DefaultLogicError', 'DefaultLogicError'};

get_response_spec('ActivateShopForParty', 401) ->
    undefined;
...
get_response_spec('GetShopByIDForParty', 400) ->
    {'DefaultLogicError', 'DefaultLogicError'};

get_response_spec('GetShopByIDForParty', 401) ->
    undefined;
...
get_response_spec('GetShopsForParty', 400) ->
    {'DefaultLogicError', 'DefaultLogicError'};

get_response_spec('GetShopsForParty', 401) ->
    undefined;
...
get_response_spec('SuspendShopForParty', 400) ->
    {'DefaultLogicError', 'DefaultLogicError'};

get_response_spec('SuspendShopForParty', 401) ->
    undefined;

get_response_spec('SuspendShopForParty', 404) ->
    {'GeneralError', 'GeneralError'};
