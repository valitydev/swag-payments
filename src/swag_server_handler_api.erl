...
-spec authorize_api_key(
    ...
    {true, swag_server:auth_context(), cowboy_req:req()} |
...
-spec populate_request(
    ...
    Spec :: request_spec(),
    Req :: cowboy_req:req(),
    ValidationOpts :: swag_server_validation:validation_opts()
) ->
...
validate_response(OperationID, Spec, RespBody, ValidationOpts) ->
    case swag_server_validation:validate_response(OperationID, Spec, RespBody, ValidationOpts) of
        ok ->
            ok;
        {error, Error} ->
...
populate_request_param(LogicHandler, OperationID, {Name, #{rules := Rules, source := Source}}, Req0, ValidationOpts) ->
    case get_value(Source, Name, Req0) of
        {ok, Value, Req} ->
            case swag_server_validation:prepare_request_param(OperationID, Rules, Name, Value, ValidationOpts) of
                {ok, Result} ->
                    {ok, Name, Result, Req};
                {error, Error}  ->
                    {error, swag_server_logic_handler:map_error(LogicHandler, {validation_error, Error}), Req}
            end;
        {error, Message, Req} ->
            Error = #{
                type => wrong_body,
...
get_value(body, _Name, Req0) ->
    case swag_server_utils:get_body(Req0) of
        {ok, Body, Req} ->
...
get_value(header, Name, Req) ->
    ...
    {ok, maps:get(swag_server_utils:to_header(Name), Headers, undefined), Req};
get_value(binding, Name, Req) ->
    Bindings = cowboy_req:bindings(Req),
    {ok, maps:get(swag_server_utils:to_binding(Name), Bindings, undefined), Req}.
