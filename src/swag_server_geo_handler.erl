...
%% basic handler
-module(swag_server_geo_handler).
...
allowed_methods(
    Req,
    State = #state{
        operation_id = 'GetLocationsNames'
    }
) ->
    {[<<"GET">>], Req, State};
...
is_authorized(
    Req0,
    State = #state{
        operation_id  = 'GetLocationsNames' = OperationID,
        logic_handler = LogicHandler,
        context       = Context
    }
) ->
...
get_request_spec('GetLocationsNames') ->
    [
        {'X-Request-ID', #{
            source => header,
...
get_response_spec('GetLocationsNames', 200) ->
    {'list', 'LocationName'};

get_response_spec('GetLocationsNames', 400) ->
    {'DefaultLogicError', 'DefaultLogicError'};

get_response_spec('GetLocationsNames', 401) ->
    undefined;
