...
%% basic handler
-module(swag_server_tokens_handler).
...
allowed_methods(
    Req,
    State = #state{
        operation_id = 'CreatePaymentResource'
    }
) ->
    {[<<"POST">>], Req, State};
...
is_authorized(
    Req0,
    State = #state{
        operation_id  = 'CreatePaymentResource' = OperationID,
        logic_handler = LogicHandler,
        context       = Context
    }
) ->
...
get_request_spec('CreatePaymentResource') ->
    [
        {'X-Request-ID', #{
            source => header,
...
get_response_spec('CreatePaymentResource', 201) ->
    {'PaymentResourceResult', 'PaymentResourceResult'};

get_response_spec('CreatePaymentResource', 400) ->
