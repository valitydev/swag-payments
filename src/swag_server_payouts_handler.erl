...
%% basic handler
-module(swag_server_payouts_handler).
...
allowed_methods(
    Req,
    State = #state{
        operation_id = 'GetPayoutToolByID'
    }
) ->
    {[<<"GET">>], Req, State};
...
allowed_methods(
    Req,
    State = #state{
        operation_id = 'GetPayoutToolByIDForParty'
    }
) ->
    {[<<"GET">>], Req, State};
...
allowed_methods(
    Req,
    State = #state{
        operation_id = 'GetPayoutToolsForParty'
    }
) ->
    {[<<"GET">>], Req, State};
...
is_authorized(
    Req0,
    State = #state{
        operation_id  = 'CreatePayout' = OperationID,
        logic_handler = LogicHandler,
        context       = Context
    }
) ->
...
is_authorized(
    Req0,
    State = #state{
        operation_id  = 'GetPayout' = OperationID,
        logic_handler = LogicHandler,
        context       = Context
    }
) ->
...
is_authorized(
    Req0,
    State = #state{
        operation_id  = 'GetPayoutToolByID' = OperationID,
        logic_handler = LogicHandler,
        context       = Context
    }
) ->
...
is_authorized(
    Req0,
    State = #state{
        operation_id  = 'GetPayoutToolByIDForParty' = OperationID,
        logic_handler = LogicHandler,
        context       = Context
    }
) ->
...
is_authorized(
    Req0,
    State = #state{
        operation_id  = 'GetPayoutTools' = OperationID,
        logic_handler = LogicHandler,
        context       = Context
    }
) ->
...
is_authorized(
    Req0,
    State = #state{
        operation_id  = 'GetPayoutToolsForParty' = OperationID,
        logic_handler = LogicHandler,
        context       = Context
    }
) ->
...
is_authorized(
    Req0,
    State = #state{
        operation_id  = 'GetScheduleByRef' = OperationID,
        logic_handler = LogicHandler,
        context       = Context
    }
) ->
...
get_request_spec('GetPayoutToolByID') ->
    [
        {'X-Request-ID', #{
            source => header,
            rules  => [{type, 'binary'}, {max_length, 32}, {min_length, 1}, true
...
get_request_spec('GetPayoutToolByIDForParty') ->
    [
        {'X-Request-ID', #{
            source => header,
            rules  => [{type, 'binary'}, {max_length, 32}, {min_length, 1}, true
...
get_request_spec('GetPayoutTools') ->
    [
        {'X-Request-ID', #{
            source => header,
            rules  => [{type, 'binary'}, {max_length, 32}, {min_length, 1}, true
...
get_request_spec('GetPayoutToolsForParty') ->
    [
        {'X-Request-ID', #{
            source => header,
            rules  => [{type, 'binary'}, {max_length, 32}, {min_length, 1}, true
...
get_request_spec('GetScheduleByRef') ->
    [
        {'X-Request-ID', #{
            source => header,
            rules  => [{type, 'binary'}, {max_length, 32}, {min_length, 1}, true
...
get_response_spec('CreatePayout', 201) ->
    {'Payout', 'Payout'};
...
get_response_spec('GetPayout', 200) ->
    {'Payout', 'Payout'};

get_response_spec('GetPayout', 400) ->
    {'DefaultLogicError', 'DefaultLogicError'};

get_response_spec('GetPayout', 401) ->
    undefined;

get_response_spec('GetPayout', 404) ->
    {'GeneralError', 'GeneralError'};

get_response_spec('GetPayoutToolByID', 200) ->
    {'PayoutTool', 'PayoutTool'};

get_response_spec('GetPayoutToolByID', 400) ->
    {'DefaultLogicError', 'DefaultLogicError'};

get_response_spec('GetPayoutToolByID', 401) ->
    undefined;

get_response_spec('GetPayoutToolByID', 404) ->
    {'GeneralError', 'GeneralError'};

get_response_spec('GetPayoutToolByIDForParty', 200) ->
    {'PayoutTool', 'PayoutTool'};

get_response_spec('GetPayoutToolByIDForParty', 400) ->
    {'DefaultLogicError', 'DefaultLogicError'};

get_response_spec('GetPayoutToolByIDForParty', 401) ->
    undefined;

get_response_spec('GetPayoutToolByIDForParty', 404) ->
    {'GeneralError', 'GeneralError'};

get_response_spec('GetPayoutTools', 200) ->
    {'list', 'PayoutTool'};

get_response_spec('GetPayoutTools', 400) ->
    {'DefaultLogicError', 'DefaultLogicError'};

get_response_spec('GetPayoutTools', 401) ->
    undefined;

get_response_spec('GetPayoutTools', 404) ->
    {'GeneralError', 'GeneralError'};

get_response_spec('GetPayoutToolsForParty', 200) ->
    {'list', 'PayoutTool'};

get_response_spec('GetPayoutToolsForParty', 400) ->
    {'DefaultLogicError', 'DefaultLogicError'};

get_response_spec('GetPayoutToolsForParty', 401) ->
    undefined;

get_response_spec('GetPayoutToolsForParty', 404) ->
    {'GeneralError', 'GeneralError'};

get_response_spec('GetScheduleByRef', 200) ->
    {'Schedule', 'Schedule'};

get_response_spec('GetScheduleByRef', 400) ->
    {'DefaultLogicError', 'DefaultLogicError'};

get_response_spec('GetScheduleByRef', 401) ->
    undefined;

get_response_spec('GetScheduleByRef', 404) ->
    {'GeneralError', 'GeneralError'};
