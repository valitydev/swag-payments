...
%% basic handler
-module(swag_server_countries_handler).
...
is_authorized(
    Req0,
    State = #state{
        operation_id  = 'GetCountries' = OperationID,
        logic_handler = LogicHandler,
        context       = Context
    }
) ->
...
is_authorized(
    Req0,
    State = #state{
        operation_id  = 'GetCountryByID' = OperationID,
        logic_handler = LogicHandler,
        context       = Context
    }
) ->
...
get_request_spec('GetCountryByID') ->
    [
        {'X-Request-ID', #{
            source => header,
            rules  => [{type, 'binary'}, {max_length, 32}, {min_length, 1}, true
...
get_response_spec('GetCountries', 200) ->
    {'list', 'Country'};

get_response_spec('GetCountries', 400) ->
    {'DefaultLogicError', 'DefaultLogicError'};

get_response_spec('GetCountryByID', 200) ->
    {'Country', 'Country'};

get_response_spec('GetCountryByID', 400) ->
    {'DefaultLogicError', 'DefaultLogicError'};

get_response_spec('GetCountryByID', 404) ->
