...
%% basic handler
-module(swag_server_trade_blocs_handler).
...
is_authorized(
    Req0,
    State = #state{
        operation_id  = 'GetTradeBlocByID' = OperationID,
        logic_handler = LogicHandler,
        context       = Context
    }
) ->
...
is_authorized(
    Req0,
    State = #state{
        operation_id  = 'GetTradeBlocs' = OperationID,
        logic_handler = LogicHandler,
        context       = Context
    }
) ->
...
get_request_spec('GetTradeBlocByID') ->
    [
        {'X-Request-ID', #{
            source => header,
...
get_request_spec('GetTradeBlocs') ->
    [
        {'X-Request-ID', #{
            source => header,
            rules  => [{type, 'binary'}, {max_length, 32}, {min_length, 1}, true
...
get_response_spec('GetTradeBlocByID', 200) ->
    {'TradeBloc', 'TradeBloc'};

get_response_spec('GetTradeBlocByID', 400) ->
    {'DefaultLogicError', 'DefaultLogicError'};

get_response_spec('GetTradeBlocByID', 404) ->
    {'GeneralError', 'GeneralError'};

get_response_spec('GetTradeBlocs', 200) ->
    {'list', 'TradeBloc'};

get_response_spec('GetTradeBlocs', 400) ->
    {'DefaultLogicError', 'DefaultLogicError'};
