...
%% basic handler
-module(swag_server_accounts_handler).
...
is_authorized(
    Req0,
    State = #state{
        operation_id  = 'GetAccountByID' = OperationID,
        logic_handler = LogicHandler,
        context       = Context
    }
) ->
...
get_request_spec('GetAccountByID') ->
    [
        {'X-Request-ID', #{
            source => header,
...
get_response_spec('GetAccountByID', 200) ->
    {'Account', 'Account'};

get_response_spec('GetAccountByID', 400) ->
    {'DefaultLogicError', 'DefaultLogicError'};

get_response_spec('GetAccountByID', 401) ->
    undefined;

get_response_spec('GetAccountByID', 404) ->
