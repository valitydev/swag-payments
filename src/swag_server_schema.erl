...
get_raw() ->
  ...
  <<"swagger">> => <<"2.0">>,
  <<"info">> => #{
    <<"description">> => <<"#### Последнее обновление: 16-09-2021 | \n\n## Описание\n\nRBK.money API предназначен для [мерчантов](https://developer.rbk.money/docs/payments/overview/#lk), принимающих\nплатежи из своего пользовательского интерфейса (веб-сайта или любого другого),\nи является единственной точкой взаимодействия с [платформой](https://developer.rbk.money/docs/payments/overview/) для проведения операций оплаты товаров/услуг.\n\n## Сценарии\n\nПримеры информационного взаимодействия для некоторых пользовательских сценариев представлены в данных статьях:\n\n  - оплата [заказа](https://developer.rbk.money/docs/payments/overview/#invoice) покупателем: см. [\"Платеж\"](https://developer.rbk.money/docs/payments/overview/#pay);\n  - [рекуррентный платёж](https://developer.rbk.money/docs/payments/recurring/recurring/#_1);\n  - [покупка с привязанной карты](https://developer.rbk.money/docs/payments/examples/binding/).\n\n## Детали взаимодействия\n\nПри любом обращении к API в заголовке `X-Request-ID` соответствующего запроса необходимо передать его уникальный идентификатор:\n\n```\n X-Request-ID: oX5MWM2AQy\n```\n\n### Тип содержимого и кодировка\n\n[Платформа](https://developer.rbk.money/docs/payments/overview/) принимает и возвращает данные в формате JSON и кодировке UTF-8:\n\n```\n  Content-Type: application/json; charset=utf-8\n```\n\n### Формат дат\n\n[Платформа](https://developer.rbk.money/docs/payments/overview/) принимает и возвращает значения отметок времени в формате `date-time`, описанном в [RFC 3339](https://datatracker.ietf.org/doc/html/rfc3339):\n\n```\n  2017-01-01T00:00:00Z\n  2017-01-01T00:00:01+00:00\n```\n\n### Максимальное время обработки запроса\n\nПри любом обращении к API в заголовке `X-Request-Deadline` соответствующего запроса можно передать параметр отсечки по времени, определяющий максимальное время ожидания завершения операции по запросу:\n\n```\n X-Request-Deadline: 10s\n```\n\nПо истечении указанного времени [платформа](https://developer.rbk.money/docs/payments/overview/) прекращает обработку запроса.\n\n`X-Request-Deadline` может:\n\n* задаваться в формате `date-time` (см. [RFC 3339](https://datatracker.ietf.org/doc/html/rfc3339));\n* задаваться в относительных величинах, в миллисекундах (`150000ms`), секундах (`540s`) или минутах (`3.5m`).\n\nРекомендуемые значения: не более одной минуты, но не менее трёх секунд.\n">>,
    <<"version">> => <<"2.0.1">>,
    <<"title">> => <<"RBK.money Platform API">>,
    <<"termsOfService">> => <<"https://rbk.money/">>,
    <<"contact">> => #{
      <<"name">> => <<"RBK.money support team">>,
  ...
  <<"tags">> => [ #{
    ...
    <<"name">> => <<"Geo">>,
    <<"description">> => <<"Для отображения геоданных плательщиков существуют вспомогательные операции, например, получение названия геопозиции по её идентификатору.\n">>,
    ...
    <<"name">> => <<"Analytics">>,
    <<"description">> => <<"Платформа предоставляет возможность получения различных аналитических данных, полученных в рамках взаимодействия с ней. Аналитические отчеты подготовлены заранее и не подразумевают интерактивного изменения структуры данных.\n">>,
  ...
  <<"paths">> => #{
    <<"/analytics/shops/{shopID}/customers/stats/payment_method">> => #{
      <<"get">> => #{
        <<"tags">> => [ <<"Analytics">> ],
        <<"description">> => <<"Получить статистику по платежным средствам за определённое время.\n">>,
    ...
    <<"/analytics/shops/{shopID}/customers/stats/rate">> => #{
      <<"get">> => #{
        <<"tags">> => [ <<"Analytics">> ],
        <<"description">> => <<"Получить статистику по уникальным плательщикам за определённое время.\n">>,
    ...
    <<"/analytics/shops/{shopID}/payments/stats/conversion">> => #{
      <<"get">> => #{
        <<"tags">> => [ <<"Analytics">> ],
        <<"description">> => <<"Получить статистику конверсии платежей магазина за определённое время.\n">>,
    ...
    <<"/analytics/shops/{shopID}/payments/stats/geo">> => #{
      <<"get">> => #{
        <<"tags">> => [ <<"Analytics">> ],
        <<"description">> => <<"Получить статистику по геопозициям плательщиков за определённое время.\n">>,
    ...
    <<"/analytics/shops/{shopID}/payments/stats/revenue">> => #{
      <<"get">> => #{
        <<"tags">> => [ <<"Analytics">> ],
        <<"description">> => <<"Получить статистику по сумме платежей магазина за определенное время, в том\nчисле за вычетом комиссии системы.\n">>,
    ...
    <<"/parties/{partyID}/shops/{shopID}/reports">> => #{
      <<"get">> => #{
        <<"tags">> => [ <<"Reports">> ],
    ...
    <<"/parties/{partyID}/shops/{shopID}/reports/{reportID}">> => #{
      <<"get">> => #{
        <<"tags">> => [ <<"Reports">> ],
    ...
    <<"/parties/{partyID}/shops/{shopID}/reports/{reportID}/files/{fileID}/download">> => #{
      <<"get">> => #{
        <<"tags">> => [ <<"Reports">> ],
    ...
    <<"/processing/claims">> => #{
      <<"get">> => #{
        <<"tags">> => [ <<"Claims">> ],
        <<"description">> => <<"Получить список заявок участника, при желании отфильтрованный по значению\nстатуса.\n">>,
        ...
        <<"parameters">> => [ #{
          ...
          <<"required">> => false,
          <<"type">> => <<"string">>,
          <<"enum">> => [ <<"pending">>, <<"accepted">>, <<"denied">>, <<"revoked">> ]
        } ],
        <<"responses">> => #{
          <<"200">> => #{
      ...
      <<"post">> => #{
        ...
        <<"responses">> => #{
          ...
          <<"400">> => #{
            ...
            <<"schema">> => #{
              <<"$ref">> => <<"#/definitions/inline_response_400_15">>
            }
          },
          <<"401">> => #{
            <<"description">> => <<"Ошибка авторизации">>
      ...
      <<"post">> => #{
        ...
        <<"responses">> => #{
          ...
          <<"400">> => #{
            ...
            <<"schema">> => #{
              <<"$ref">> => <<"#/definitions/inline_response_400_16">>
            }
          },
          <<"401">> => #{
            <<"description">> => <<"Ошибка авторизации">>
      ...
      <<"post">> => #{
        ...
        <<"responses">> => #{
          ...
          <<"400">> => #{
            <<"description">> => <<"Неверные данные">>,
            <<"schema">> => #{
              <<"$ref">> => <<"#/definitions/inline_response_400_17">>
            }
          },
          <<"401">> => #{
            <<"description">> => <<"Ошибка авторизации">>
      ...
      <<"post">> => #{
        ...
        <<"responses">> => #{
          ...
          <<"400">> => #{
            ...
            <<"schema">> => #{
              <<"$ref">> => <<"#/definitions/inline_response_400_18">>
            }
          },
          <<"401">> => #{
            <<"description">> => <<"Ошибка авторизации">>
    ...
    <<"/reference/geo/location/names">> => #{
      <<"get">> => #{
        <<"tags">> => [ <<"Geo">> ],
        <<"description">> => <<"Получить локализованные названия геопозиций плательщиков по их\nидентификаторам.\n">>,
  ...
  <<"securityDefinitions">> => #{
    <<"bearer">> => #{
      <<"description">> => <<"Взаимодействие между [мерчантом](https://developer.rbk.money/docs/payments/overview/#lk) и [платформой](https://developer.rbk.money/docs/payments/overview/) осуществляется по защищенному протоколу (HTTPS). HTTP-запросы по нешифрованному каналу не поддерживаются.\n\nОзнакомиться с деталями авторизации запросов к API можно в [данной](https://developer.rbk.money/docs/payments/overview/#api) статье.\n\n[Посмотреть](https://help.rbkmoney.com/lk/lk/#apiKey) ваш API-ключ можно в [личном кабинете](https://developer.rbk.money/docs/payments/overview/#lk). Значение ключа следует передать в заголовке `Authorization`:\n\n  ```\n  Authorization: Bearer MjMyNDQxMjM6NDUzRmRnZDQ0M*******\n  ```\n\nКлючи не разделяются на тестовые и боевые. Для проведения тестовых транзакций используйте идентификатор [тестового магазина](https://help.rbkmoney.com/lk/lk/#testManagement).\n\n**Важно! Не передавайте API-ключ третьим лицам!**\n">>,
      <<"type">> => <<"apiKey">>,
      <<"name">> => <<"Authorization">>,
      <<"in">> => <<"header">>
    }
  ...
  <<"definitions">> => #{
    ...
    <<"Account">> => #{
      <<"type">> => <<"object">>,
      <<"required">> => [ <<"availableAmount">>, <<"currency">>, <<"id">>, <<"ownAmount">> ],
      <<"properties">> => #{
        <<"id">> => #{
          <<"type">> => <<"integer">>,
          <<"format">> => <<"int64">>
    ...
    <<"Claim">> => #{
      <<"type">> => <<"object">>,
      <<"required">> => [ <<"changeset">>, <<"createdAt">>, <<"id">>, <<"revision">>, <<"status">> ],
    ...
    <<"ContractCreation">> => #{
      <<"allOf">> => [ #{
        ...
        <<"type">> => <<"object">>,
        <<"required">> => [ <<"contractor">>, <<"paymentInstitutionID">> ],
        <<"properties">> => #{
          <<"contractor">> => #{
            <<"$ref">> => <<"#/definitions/Contractor">>
          },
    ...
    <<"ContractModification">> => #{
      <<"allOf">> => [ #{
        ...
        <<"type">> => <<"object">>,
        <<"required">> => [ <<"contractID">>, <<"contractModificationType">> ],
        ...
        <<"properties">> => #{
          ...
          <<"contractModificationType">> => #{
            <<"type">> => <<"string">>,
            <<"enum">> => [ <<"ContractCreation">>, <<"ContractTermination">>, <<"ContractLegalAgreementBinding">>, <<"ContractAdjustmentCreation">>, <<"ContractPayoutToolCreation">>, <<"ContractPayoutToolInfoModification">>, <<"ContractReportingPreferencesChange">> ]
          }
        }
      } ],
    ...
    <<"PartyModification">> => #{
      <<"type">> => <<"object">>,
      <<"required">> => [ <<"partyModificationType">> ],
      <<"discriminator">> => <<"partyModificationType">>,
      <<"properties">> => #{
        <<"partyModificationType">> => #{
          <<"type">> => <<"string">>,
          <<"enum">> => [ <<"ContractModification">>, <<"ShopModification">> ]
        }
      },
    ...
    <<"PaymentConversionStat">> => #{
      <<"type">> => <<"object">>,
      <<"required">> => [ <<"conversion">>, <<"offset">>, <<"successfulCount">>, <<"totalCount">> ],
      <<"properties">> => #{
        <<"offset">> => #{
          <<"type">> => <<"integer">>,
      ...
      <<"example">> => #{
        ...
        <<"successfulCount">> => 6,
        <<"totalCount">> => 1,
        <<"conversion">> => 5.962133916683182377482808078639209270477294921875
      }
    },
    <<"PaymentError">> => #{
      <<"type">> => <<"object">>,
    ...
    <<"PaymentGeoStat">> => #{
      <<"type">> => <<"object">>,
      <<"required">> => [ <<"currency">>, <<"geoID">>, <<"offset">>, <<"profit">>, <<"revenue">> ],
      <<"properties">> => #{
        <<"offset">> => #{
          <<"type">> => <<"integer">>,
          <<"description">> => <<"Смещение сегмента выборки">>
    ...
    <<"PaymentMethodBankCardStat">> => #{
      <<"allOf">> => [ #{
        ...
        <<"type">> => <<"object">>,
        <<"required">> => [ <<"offset">>, <<"paymentSystem">>, <<"profit">>, <<"revenue">>, <<"totalCount">> ],
        <<"properties">> => #{
          <<"offset">> => #{
            <<"type">> => <<"integer">>,
            <<"description">> => <<"Смещение сегмента выборки">>
    ...
    <<"PaymentRevenueStat">> => #{
      <<"type">> => <<"object">>,
      <<"required">> => [ <<"currency">>, <<"offset">>, <<"profit">>, <<"revenue">> ],
      <<"properties">> => #{
        <<"offset">> => #{
          <<"type">> => <<"integer">>,
          <<"description">> => <<"Смещение сегмента выборки">>
    ...
    <<"ShopContractBinding">> => #{
      <<"allOf">> => [ #{
        ...
        <<"type">> => <<"object">>,
        <<"required">> => [ <<"contractID">>, <<"payoutToolID">> ],
        <<"properties">> => #{
          <<"contractID">> => #{
            <<"type">> => <<"string">>,
            <<"description">> => <<"Идентификатор договора, на основании которого следует производить\nобслуживание магазина\n">>
          },
          <<"payoutToolID">> => #{
            <<"type">> => <<"string">>,
            <<"description">> => <<"Идентификатор средства вывода в рамках указанного контракта\n">>
          }
        }
      } ],
    ...
    <<"ShopCreation">> => #{
      <<"allOf">> => [ #{
        ...
        <<"type">> => <<"object">>,
        <<"required">> => [ <<"contractID">>, <<"details">>, <<"location">>, <<"payoutToolID">> ],
        <<"properties">> => #{
          <<"location">> => #{
            <<"$ref">> => <<"#/definitions/ShopLocation">>
          },
          ...
          <<"payoutToolID">> => #{
            <<"type">> => <<"string">>,
            <<"description">> => <<"Идентификатор средства вывода для создаваемого магазина в рамках\nуказанного контракта\n">>
          }
        }
      } ],
    ...
    <<"ShopModification">> => #{
      <<"allOf">> => [ #{
        ...
        <<"type">> => <<"object">>,
        <<"required">> => [ <<"shopID">>, <<"shopModificationType">> ],
        ...
        <<"properties">> => #{
          ...
          <<"shopModificationType">> => #{
            <<"type">> => <<"string">>,
            <<"enum">> => [ <<"ShopCreation">>, <<"ShopAccountCreation">>, <<"ShopCategoryChange">>, <<"ShopLocationChange">>, <<"ShopDetailsChange">>, <<"ShopContractBinding">>, <<"ShopPayoutToolChange">>, <<"ShopPayoutScheduleChange">> ]
          }
        }
      } ]
    },
    ...
    <<"ShopPayoutToolChange">> => #{
      <<"allOf">> => [ #{
        ...
        <<"properties">> => #{
          <<"payoutToolID">> => #{
            <<"type">> => <<"string">>,
            <<"description">> => <<"Идентификатор средства вывода в рамках текущего контракта магазина\n">>
          }
        }
      } ],
    ...
    <<"inline_response_400">> => #{
      ...
      <<"properties">> => #{
        <<"code">> => #{
          <<"type">> => <<"string">>,
          <<"description">> => <<"[Код ошибки](#tag/Error-Codes)\n">>,
          <<"enum">> => [ <<"limitExceeded">>, <<"invalidRequest">>, <<"invalidDeadline">> ]
        },
        <<"message">> => #{
          <<"type">> => <<"string">>,
    ...
    <<"inline_response_400_1">> => #{
      ...
      <<"properties">> => #{
        <<"code">> => #{
          <<"type">> => <<"string">>,
          <<"description">> => <<"[Код ошибки](#tag/Error-Codes)\n">>,
          <<"enum">> => [ <<"invalidShopID">>, <<"invalidRequest">>, <<"invalidDeadline">> ]
        },
        <<"message">> => #{
          <<"type">> => <<"string">>,
    ...
    <<"inline_response_400_2">> => #{
      ...
      <<"properties">> => #{
        <<"code">> => #{
          <<"type">> => <<"string">>,
          <<"description">> => <<"[Код ошибки](#tag/Error-Codes)\n">>,
          <<"enum">> => [ <<"changesetConflict">>, <<"invalidPartyStatus">>, <<"invalidChangeset">>, <<"invalidRequest">>, <<"invalidDeadline">> ]
        },
        <<"message">> => #{
          <<"type">> => <<"string">>,
    ...
    <<"inline_response_400_3">> => #{
      ...
      <<"properties">> => #{
        <<"code">> => #{
          <<"type">> => <<"string">>,
          <<"description">> => <<"[Код ошибки](#tag/Error-Codes)\n">>,
          <<"enum">> => [ <<"invalidClaimStatus">>, <<"invalidClaimRevision">>, <<"invalidPartyStatus">>, <<"invalidRequest">>, <<"invalidDeadline">> ]
        },
        <<"message">> => #{
          <<"type">> => <<"string">>,
