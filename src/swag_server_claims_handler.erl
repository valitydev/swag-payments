...
%% basic handler
-module(swag_server_claims_handler).
...
is_authorized(
    Req0,
    State = #state{
        operation_id  = 'CreateClaim' = OperationID,
        logic_handler = LogicHandler,
        context       = Context
    }
) ->
...
is_authorized(
    Req0,
    State = #state{
        operation_id  = 'GetClaimByID' = OperationID,
        logic_handler = LogicHandler,
        context       = Context
    }
) ->
...
is_authorized(
    Req0,
    State = #state{
        operation_id  = 'GetClaims' = OperationID,
        logic_handler = LogicHandler,
        context       = Context
    }
) ->
...
is_authorized(
    Req0,
    State = #state{
        operation_id  = 'RevokeClaimByID' = OperationID,
        logic_handler = LogicHandler,
        context       = Context
    }
) ->
...
get_request_spec('RevokeClaimByID') ->
    [
        {'X-Request-ID', #{
            source => header,
            rules  => [{type, 'binary'}, {max_length, 32}, {min_length, 1}, true
...
get_response_spec('CreateClaim', 201) ->
    {'Claim', 'Claim'};

get_response_spec('CreateClaim', 400) ->
    {'inline_response_400_2', 'inline_response_400_2'};

get_response_spec('CreateClaim', 401) ->
    undefined;

get_response_spec('GetClaimByID', 200) ->
    {'Claim', 'Claim'};

get_response_spec('GetClaimByID', 400) ->
    {'DefaultLogicError', 'DefaultLogicError'};

get_response_spec('GetClaimByID', 401) ->
    undefined;

get_response_spec('GetClaimByID', 404) ->
    {'GeneralError', 'GeneralError'};

get_response_spec('GetClaims', 200) ->
    {'list', 'Claim'};

get_response_spec('GetClaims', 400) ->
    {'DefaultLogicError', 'DefaultLogicError'};

get_response_spec('GetClaims', 401) ->
    undefined;

get_response_spec('RevokeClaimByID', 204) ->
    undefined;

get_response_spec('RevokeClaimByID', 400) ->
    {'inline_response_400_3', 'inline_response_400_3'};

get_response_spec('RevokeClaimByID', 401) ->
    undefined;

get_response_spec('RevokeClaimByID', 404) ->
    {'GeneralError', 'GeneralError'};
