...
%% basic handler
-module(swag_server_categories_handler).
...
is_authorized(
    Req0,
    State = #state{
        operation_id  = 'GetCategories' = OperationID,
        logic_handler = LogicHandler,
        context       = Context
    }
) ->
...
is_authorized(
    Req0,
    State = #state{
        operation_id  = 'GetCategoryByRef' = OperationID,
        logic_handler = LogicHandler,
        context       = Context
    }
) ->
...
get_request_spec('GetCategories') ->
    [
        {'X-Request-ID', #{
            source => header,
...
get_request_spec('GetCategoryByRef') ->
    [
        {'X-Request-ID', #{
            source => header,
            rules  => [{type, 'binary'}, {max_length, 32}, {min_length, 1}, true
...
get_response_spec('GetCategories', 200) ->
    {'list', 'Category'};

get_response_spec('GetCategories', 400) ->
    {'DefaultLogicError', 'DefaultLogicError'};

get_response_spec('GetCategories', 401) ->
    undefined;

get_response_spec('GetCategoryByRef', 200) ->
    {'Category', 'Category'};

get_response_spec('GetCategoryByRef', 400) ->
    {'DefaultLogicError', 'DefaultLogicError'};

get_response_spec('GetCategoryByRef', 401) ->
    undefined;

get_response_spec('GetCategoryByRef', 404) ->
